...
if (Get<string>("SelectedListings").Length > 0) {
...
using (Ionic.Zip.ZipFile zip = new Ionic.Zip.ZipFile()) {
	...
	contactsEmail.Add("autoload@avito.ru");
	...
	ftp.CreateFileFromContent(ssFTPPath, ssFTPObjectsFilename, buffer, buffer.Length);
	ftp.CreateFileFromContent(ssFTPPath, ssFTPArchiveFilename, archArray, archArray.Length);
