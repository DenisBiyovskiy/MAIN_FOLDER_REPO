			...
			checkOwnerWorkPhone(function() {
				var serviceConfig = {
					...
					callback: function(response) {
						...
						if (result.isValid) {
							var count = this.changedValues.ActiveRow != null ? 1 : this.changedValues.SelectedRows.length;
							var message = "Опубликовано на Zipal " + count;
							if (count == 1) message += " листинг.";
							else if (count < 5) message += " листинга.";
							else message += " листингов.";
							if (count == 1) {
								var batchQuery = this.Ext.create("Terrasoft.BatchQuery");
								var update = this.Ext.create("Terrasoft.UpdateQuery", {
									rootSchemaName: "Listing"
								});
								update.enablePrimaryColumnFilter(this.changedValues.ActiveRow);
								update.setParameterValue("UsrLastExportZipal", new Date(), this.Terrasoft.DataValueType.DATE_TIME);
								update.setParameterValue("UsrExportedOnZipal", true, this.Terrasoft.DataValueType.BOOLEAN);
								batchQuery.add(update);
								batchQuery.execute(function () {
								}, this);
							} else {
								var selectedRows = this.changedValues.SelectedRows;
				...
				callback: function(response) {
					...
					if (this.Ext.isString(result)) {
						if (this.Ext.isEmpty(result)) {
							var count = this.changedValues.ActiveRow != null ? 1 : this.changedValues.SelectedRows.length;
							var message = "Снято с Zipal " + count;
							if (count == 1) message += " листинг.";
							else if (count < 5) message += " листинга.";
							else message += " листингов.";
							if (count == 1) {
								...
								update.enablePrimaryColumnFilter(this.changedValues.ActiveRow);
								update.setParameterValue("UsrStopZipal", new Date(), this.Terrasoft.DataValueType.DATE_TIME);
								update.setParameterValue("UsrExportedOnZipal", false, this.Terrasoft.DataValueType.BOOLEAN);
								batchQuery.add(update);
								batchQuery.execute(function () {
								}, this);
							} else {
