define("CtiPanelModelUtilities", ["CtiConstants", "NetworkUtilities"],
	function(CtiConstants, NetworkUtilities) {
		...
		Ext.define("Terrasoft.configuration.mixins.CtiPanelModelUtilities", {
			 ...
			 * @return {Object} Локализированное изображение.
			 */
			...
			/**
			 * Функция обратного вызова идентификации абонента при изменении номера основного или консультационного
			 * звонка.
			 * @private
			 * @param {String} number Номер абонента.
			 * @param {String} identifyPropertyName Название свойства модели, по которому проводилась идентификация.
			 * @param {Object[]} subscribers Массив идентифицированных абонентов.
			 * @param {String} [collectionName] Название коллекции идентифицируемых абонентов.
			 * @param {String} [subscriberKeyName] Название свойства с ключом идентифицированного абонента.
			 */
			 ...
			 * @protected
			 * @return {Boolean} true, если можно позвонить либо совершить консультационный звонок. Иначе - false.
			 */
			 ...
			 * @protected
			 * @return {Boolean} Признак видимости контейнера с найденными абонентами в результате поиска.
			 */
			 ...
			 * @private
			 * @return {Boolean} true, если нет активного звонка и нет текста в поле поиска абонентоа. Иначе - false.
			 */
			 ...
			 * @protected
			 * @return {Boolean} true, если можно совершить DTMF набор. Иначе - false.
			 */
			 ...
			 * @protected
			 * @return {Boolean} true, если доступна инициация либо завершение перевода, иначе - false.
			 */
			...
			/**
			 * Возвращает конфигурацию изображения кнопки отображения/скрытия поля ввода DTMF набора.
			 * @protected
			 * @return {Object} Конфигурация изображения.
			 */
			...
			getSubscriberPhoto: function(tag, collectionName, subscriberKeyName) {
				this.isVipSignVisible();
				this.isToughCaseSignVisible();
			 ...
			 * @return {String} Информация об абоненте.
			 */
			 ...
			 * @return {Boolean} Абонент идентифицирован.
			 */
			 ...
			 * @protected
			 * @return {Boolean} true, если звонок существует. Иначе - false.
			 */
			 ...
			 * @private
			 * @return {Boolean} true, если нет записей удовлетворяющих условиям поиска. Иначе - false.
			 */
			...
			/**
			 * Обработчик нажатия кнопки DTMF набора.
			 * @private
			 */
			enterDtmf: function() {
			...
			onCallCustomer: function(numberInfo) {
				...
				//Den>[IL-485] Запоминать лид\активность при звонке
				//В numberInfo передается так же Id лида, или активности, 
				//если звонок совершен по кастомной кнопке с лида или активности.
				if (numberInfo.ilayCallLeadId) Terrasoft.CtiModel.set("ilayCallLeadId", numberInfo.ilayCallLeadId);
				if (numberInfo.ilayCallActivityId) Terrasoft.CtiModel.set("ilayCallActivityId", numberInfo.ilayCallActivityId);
				//Den<[IL-485]
			...
			/**
			 * Срабатывает при добавления звонка в список активных @link Terrasoft.CtiModel#activeCalls.
			 * @private
			 * @param {Terrasoft.integration.telephony.Call} call Звонок.
			 */
			onCtiPanelActiveCallAdded: function(call) {
				//Den>[IL-485] Запоминать лид\активность при звонке
				//TODO: добавить запись в свонок по IntegrationId = call.id в "Call"
				//Terrasoft.CtiModel.ilayActivityId
				//Terrasoft.CtiModel.ilayLeadId
				//Не забыть обнулять их после использования. (delete Terrasoft.CtiModel.ilayLeadId...)
				var ctiModel = Terrasoft.CtiModel,
					ia = ctiModel.get("ilayCallActivityId"),
					il = ctiModel.get("ilayCallLeadId");
				if ((ia || il) && call.id) {
					var update = Ext.create("Terrasoft.UpdateQuery", {
						rootSchemaName: "Call"
					}); 
					if (ia) 
						update.setParameterValue("Activity", ia, Terrasoft.DataValueType.GUID);
					if (il) 
						update.setParameterValue("Lead", il, Terrasoft.DataValueType.GUID);
					var filter = update.createColumnFilterWithParameter(this.Terrasoft.ComparisonType.EQUAL, "IntegrationId", call.id);
					update.filters.add("IdFilter", filter);
					update.execute(function(result) {
						ctiModel.set("ilayCallActivityId", null);
						ctiModel.set("ilayCallLeadId", null);
					}, this);
				}
				//Den<[IL-485]
			...
			/**
			 * Обрабатывает событие {@link CtiPanel#event-subscriberIdentified}.
			 */
			onSubscriberIdentified: function(callId, data) {
				this.updateHistoryOnSubscriberIdentified(callId, data);
			},
			
			isVipSignVisible: function() {
				var subscriberPanel = this.getIdentifiedSubscriberPanel(undefined, undefined);
				if (!subscriberPanel) {
					return false;
				}
				var isVisible = subscriberPanel.get("SignVip");
				if (!isVisible){
					return false;
				} else return isVisible;
			},
			isToughCaseSignVisible: function() {
				var subscriberPanel = this.getIdentifiedSubscriberPanel(undefined, undefined);
				if (!subscriberPanel) {
					return false;
				}
				var isVisible = subscriberPanel.get("SignToughCase");
				if (!isVisible){
					return false;
				} else return isVisible;
			}
