...
// Initialization
					...
					loadGridData: function(rowCount, callback) {//Den 
						...
						// select.addColumn("[ilayPriceForService:ilayService:Id].ilayPackage","ilayPackageObj");//Den
						// select.addColumn("[ilayPriceForService:ilayService:Id].ilayPackage.Name","ilayPackageName111");//Den
						// select.filters.addItem(Terrasoft.createColumnIsNotNullFilter("[ilayPriceForService:ilayService:Id].ilayPackage"));
						// select.filters.addItem(Terrasoft.createColumnFilterWithParameter(
						// 					9, "ilayPackageName111", "Огл"));
						// /*
						...
						basket.each(function(item) {
							...
							if (products.length) {
								...
								Terrasoft.each(products, function(pr) {
									...
									//Den>
									var packageId = item.get("ilayPackage") ? item.get("ilayPackage").value : null;
									var customKey = detailRecordId && packageId ? productId + "_" + detailRecordId + "_" + packageId : productId;
									//Den<
