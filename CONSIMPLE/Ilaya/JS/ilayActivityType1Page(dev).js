define("ilayActivityType1Page", ["ilayActivityType1PageResources", "GeneralDetails", "ilayConfigurationConstants",
	...
	"CtiPanelModelUtilities", "css!ilayScheduleCSS", "css!ilaySignsCustomCSS", "css!RecommendationModule", "css!ilayCustomStyles"],
	...
	return {
		...
		diff: /**SCHEMA_DIFF*/[
			...
			},
			{
				...
				"values": {
					...
					"bindTo": "ilayMainPhone",
					"caption": {
						"bindTo": "Resources.Strings.ilayMainPhoneCaption"
					},
					"textSize": 0,
					"contentType": 1,
					"visible": true,
			...
			},
			{
				...
				"values": {
					...
					"caption": {"bindTo": "Resources.Strings.ilayReasonCancelVisitCaption"},
					"enabled": true
				},
			...
			},
			{
				...
				"values": {
					...
					"caption": {"bindTo": "Resources.Strings.ilayContactCancelVisitCaption"},
					"enabled": true
				},
				"parentName": "groupCancel_gridLayout",
				"propertyName": "items",
			...
			},
			{
				...
				"values": {
					...
					"caption": {"bindTo": "Resources.Strings.ilayDateCancelVisitCaption"},
					"enabled": true
				},
				"parentName": "groupCancel_gridLayout",
				"propertyName": "items",
		...
		attributes: {
			...
			*/
			...
			"DurationHandler": {
				dependencies: [
					{
						columns: ["ActivityCategory", "ilayMedDirection"],
		...
		methods: {
			...
			ilayMainPhoneChanged: function() {
				this.set("CallButtonVisible", !!this.get("ilayMainPhone"));
			},
			initContactMainPhone: function() {
				var phone = this.get("ilayMainPhone");
				var patient = this.get("ilayPatient");
				if(!Ext.isEmpty(phone)){
					this.set("CallButtonVisible", true);
				}else{
					if(!Ext.isEmpty(patient)){
						...
						var filterId = select.createColumnFilterWithParameter(Terrasoft.ComparisonType.EQUAL,
							"Id", this.get("ilayPatient").value);
						select.filters.add("filterId", filterId);
						select.getEntityCollection(function(result) {
						if (result.success) {
							if (result.collection.getCount()) {
								this.set("ilayMainPhone", result.collection.getByIndex(0).get("ilayMainPhine"));
							}else{
								this.set("CallButtonVisible", false);
							}
						}
					}, this);
					}else{
			...
			makeCall: function() {
				var number = this.get("ilayMainPhone");
				var contact = this.get("ilayPatient").value;
				var entitySchemaName = "Contact";
				var ilayCallActivityId = this.get("Id");
				this.sandbox.publish("CallCustomer", {
					number: number,
					customerId: contact,
					entitySchemaName: entitySchemaName,
					ilayCallActivityId: ilayCallActivityId
				});
			},
			// + vlad [IL-]
			...
			showInformationForRecomend: function() {
				...
				var message = "Оберіть спосіб запису пацієнта по кожній рекомендації";
				this.Terrasoft.utils.showInformation(message, this.Terrasoft.emptyFn, this, buttonsConfig);
			},
			//- vlad [IL-382]
			...
			*/
				...
				if (Ext.isEmpty(patientObj)) {
					...
					this.openVisitExist(patientObj.value, function(exist, startDate) {
						if (exist) {
							...
							var that = this;
							var message = "У пацієнта є незавершений візит. Дата початку візиту: " + startDate.toLocaleString("ru");
			...
			onMoveVisitButtonClick: function() {
				if (this.isNew) {
					...
					var message = "Спочатку потрібно зберегти візит";
					this.Terrasoft.utils.showInformation(message, this.Terrasoft.emptyFn, this, buttonsConfig);
					return;
				}
				this.loadLookupDisplayValue("ilayVisitStatus", "C7ED94DC-F509-4403-B618-38829CB1E817");
				this.save({
					isCustomSilent: false
				});
				var data = this.prepareDataForStorage();
				this.setDataIntoStorage(data);
			},
			setDataIntoStorage: function(data) {
				sessionStorage.setItem("cashedDataFromVisit", JSON.stringify(data));
				var schedulerHash = "SectionModuleV2/ActivitySectionV2";
				var paramHash = "&isMoveVisit=true";
				MaskHelper.ShowBodyMask();
			...
			prepareDataForStorage: function() {
				var allData = [];
				var cols = this.entitySchema.columns;
				_.each(cols, function(col) {
					//отбрасываем пустые поля (не работает для типов GUID и TEXT)
					if (!this.model.has(col.name)) {
						delete cols[col.name];
					} else {
						allData.push({
							name: col.name,
							val: this.get(col.name).value || this.get(col.name),
							type: col.dataValueType === 10 ? 0 : col.dataValueType
						});
					}
				}, this);
				// отбрасываем пустые поля (GUID и TEXT) и ненужные для инсерта
				var reservedCols = ["Id", "CreatedOn", "CreatedBy", "ilayVisitStatus", "Status", "DueDate", "StartDate", 
					"ilaySession", "ilayCabinet", "ilayAssistant", "Owner", "Color", "ModifiedOn", "ModifiedBy", "ilayMainVisit"];
				var validData = _.filter(allData, function(item) {
					return item.val !== "" && !(_.contains(reservedCols, item.name)); 
				});
				var cachObj = {
					arr: validData,
					parentId: this.getPrimaryColumnValue()
				}
				return cachObj;
			},
			// - vlad [IL-178]
			...
			// + vlad [IL-336]
			// сначала проверяем валидность введенного времени исходя из тек. сессии validateStartDate, validateDueDate,
			// потом уже продолжаем соотв. функции-обработчики после изменения времени (calcDurationHandler и dueDateChanged соотв.)
			validateDueDate: function(){
				var clmn = "DueDate";
				if(Ext.isEmpty(this.get(clmn)) || Ext.isEmpty(this.get("ilaySession"))){
					return;
				}
				var cb = function(){
					this.dueDateChanged();
				};
				this.isTimeFitsCurrentSession(clmn, cb);
			},
			...
			isTimeFitsCurrentSession: function(clmn, callback){
				var select = this.Ext.create("Terrasoft.EntitySchemaQuery", {
					rootSchemaName: "Activity"
				});
				select.addColumn("StartDate");
				select.addColumn("DueDate");
				var filterId = select.createColumnFilterWithParameter(Terrasoft.ComparisonType.EQUAL, "Id", this.get("ilaySession").value);
				select.filters.add("filterId", filterId);
				select.getEntityCollection(function(result) {
					if (result.success) {
						if (result.collection.getCount()) {
							var sDate = result.collection.getByIndex(0).get("StartDate");
							var dDate = result.collection.getByIndex(0).get("DueDate");
							if(this.get("StartDate") < sDate || this.get("DueDate") > dDate){
								var buttonsConfig = {
									buttons: [this.Terrasoft.MessageBoxButtons.OK.returnCode],
									defaultButton: 0
								};
								var message = "Обрано час візиту, який не співпадає з часом сеансу";
								this.Terrasoft.utils.showInformation(message, this.Terrasoft.emptyFn, this, buttonsConfig);
								this.set(clmn, this.model.previous(clmn));
							}else{
								callback.call(this);
							}
						}
			...
			calcDurationHandler: function() {
				...
				var filter1 = select.createColumnFilterWithParameter(Terrasoft.ComparisonType.EQUAL, "ilayVisitCategory", category.Id);
				var filter2 = select.createColumnFilterWithParameter(Terrasoft.ComparisonType.EQUAL, "ilayMedDirection", medDirect.value);
